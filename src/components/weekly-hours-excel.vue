  ...
  <v-card class="wrapper">
    <v-card>
      ...
      <v-row>
        ...
        <v-btn @click="createWeeklyHoursToMossad()" color="success">ייצא לאקסל</v-btn>
...
export default {
  ...
  methods: {
    async createWeeklyHoursToMossad() {
      await this.getAllEmpInfo();
      await this.getAllEmpHours();
      this.setWeeklyHoursAndExport();
    },
    async createWeeklyHoursToEmployee(empId) {
      await this.getEmpInfo(empId);
      await this.getEmpHours(empId);
      this.setWeeklyHoursAndExport();
    },
